 ...
 * @author sea
 */
public abstract class AbstractMessageResource implements MessageResource {
    // 默认的国际化文件地址
    protected final static String DEFAULT_RESOURCE_FILE = "message%s.properties";
    // 默认的资源配置
    ...
    protected void loadResource() {
        defaultProperties = (Properties) loadResource(String.format(DEFAULT_RESOURCE_FILE, "")).clone();
    }
    ...
    public String getMessage(MessageCode code, Locale language, String... args) {
        String defaultLanguage = language == null ? null : language.toString();
        Properties prop = null;
        if (languageProps.containsKey(defaultLanguage)) {
            prop = languageProps.get(defaultLanguage);
        } else {
            prop = loadResource(String.format(DEFAULT_RESOURCE_FILE,
                    language == null ? "" : new StringBuilder().append("_").append(language.toString())));
     ...
     * @return 提示信息
     */
    private String getMessage(MessageCode code, Properties prop, Locale local, String... args) {
        Locale locale = local;
        if (code == null) {
     ...
     * 加载指定的国际化文件
     *
     * @param resourceFile 指定的国际化文件
     * @return 国际化文件消息属性列表
     */
    protected Properties loadResource(String resourceFile) {
        Properties props = new Properties();
        try {
            props = PropertiesLoaderUtils.loadAllProperties(resourceFile);
        } catch (IOException e) {
